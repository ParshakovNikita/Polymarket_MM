...
class Polymarket_MM():
    ...
    def __init__(self, path_to_args):
        ...
        self.polymarket_connector = Polymarket_connector(self.path_to_args)
        self.database = Database(self.path_to_args)
        self.condition_ids = self.database.read_condition_ids()
        self.bet_api = BettingAPI(path_to_args)

        self.markets = []
        for id in self.condition_ids:
            self.markets.append(Market(self.polymarket_connector.client, id))
